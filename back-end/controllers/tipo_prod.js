...
const Tipo_prod = require('../models/Tipo_prod')
...
controller.novo = async (req, res) => {
    ...
    try {
        await Tipo_prod.create(req.body)
        // HTTP 201: Created
        res.status(201).end()
    }
    catch(erro) {
...
controller.obterUm = async (req, res) => {
    try {
        ...
        let obj = await Tipo_prod.findById(id)
...
controller.atualizar = async (req, res) => {
    try {
        ...
        let ret = await Tipo_prod.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
    try {
        ...
        let ret = await Tipo_prod.findByIdAndDelete(id)
